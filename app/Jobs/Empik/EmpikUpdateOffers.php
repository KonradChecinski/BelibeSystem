...
class EmpikUpdateOffers implements ShouldQueue, ShouldBeUnique
{
    ...
    public function handle(): void
    {
        $products = Product::query()->whereHas("b2cStat", function ($query) {
            $query->where("update_in_empik", 1);
        })->get();
        if ($products->isNotEmpty()) {
            $response = Empik::updateOffers($products);

            if (!$response->successful()) {
                $this->fail('updating offers failed');
            }

            foreach ($products as $product) {
                $product->b2cStat->update_in_empik = 0;
