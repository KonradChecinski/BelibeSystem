...
class UpdateSettlementsFromSubiekt implements ShouldQueue, ShouldBeUnique
{
    ...
    {
        $updatedSettlements = DB::connection("subiekt")->table("Belibe_System_Finanse_Updated")->get();
...
//        dd($updatedSettlements);
        foreach ($updatedSettlements as $updatedSettlement) {
            ...
            if (is_null($client)) {
                DB::connection("subiekt")->table("Belibe_System_Finanse_Updated")->where("id", $updatedSettlement->id)->delete();
                continue;
            }

            $settlement = ClientSettlement::query()->where("subiekt_id", $subiektSettlement->nzf_Id)->first();
