...
class CreateKFvFromPartnerSummaryFile implements ShouldQueue, ShouldBeUnique
{
     ...
     */
    public function __construct(Partner $partner, Collection $returned)
    {
        $this->onQueue('sfera');
...
//    }
    ...
    public function handle(): void
    {
        ...
        foreach ($this->returned->where("Symbol", "S-2450-2454-1-L") as $item) {
            $item = (object)$item;
            $product = Product::query()->where("symbol", $item->Symbol)->firstOrFail();
            $queryResult = SubiektQueries::whatRemainInKFS(46, $product->subiekt_id, 2136);
            dd($item, $product, $queryResult->toArray());
        }
