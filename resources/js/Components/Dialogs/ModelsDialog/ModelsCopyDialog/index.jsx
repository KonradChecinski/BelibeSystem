...
export default function ModelsCopyDialog({open, setOpen, reloadData, modelId}) {
    const {
        register,
        handleSubmit,
        errors: fieldErrors,
    ...
    const save = () => {
        post(route("system.products.models.copy", {productModel: modelId}),

            {
                ...
                onSuccess: () => {
                    reset();
                    setActiveStep(0);
                    enqueueSnackbar("Powielono model", {variant: 'success'})
                    reloadData();
                    handleClose();
                },
                onError: errors => {
                    enqueueSnackbar("Błąd przy powielaniu modelu", {variant: 'error'})
